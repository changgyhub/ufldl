...
numFeatures = 324;    % number of features to learn  (originally set to 121)
patchDim = 16;         % patch dimension  (originally set to 8)
...
batchNumPatches = 2000; %divide into 10 batch
...
% Note: we modify the normalization process in sampleIMAGES.m to obtain a
%       reasonable result. Because the process of:

%             01  % Rescale from [-1,1] to [0.1,0.9]
%             02  patches = (patches + 1) * 0.4 + 0.1;       (in sampleIMAGES.m)

%       will make the average to 0.4. In Bayesian view, A in the model
%       shoule be random variables with an average of 0, so that the
%       product "As" will also have an average of 0.
...
[cost, grad] = sparseCodingFeatureCost(isTopo, weightMatrix, featureMatrix, visibleSize, numFeatures, patches, gamma, lambda, epsilon, groupMatrix);

numgrad = computeNumericalGradient( @(x) sparseCodingFeatureCost(isTopo, weightMatrix, x, visibleSize, numFeatures, patches, gamma, lambda, epsilon, groupMatrix), featureMatrix(:) );
% Uncomment the blow line to display the numerical and analytic gradients side by side
% disp([numgrad grad]); 
diff = norm(numgrad-grad)/norm(numgrad+grad);
...
addpath minFunc/
options.Method = 'cg';  % (originally lbfgs)
options.display = 'off';
options.verbose = 0;
...
if isequal(questdlg('Initialize grouping matrix for topographic or non-topographic sparse coding?', 'Topographic/non-topographic?', 'Non-topographic', 'Topographic', 'Non-topographic'), 'Non-topographic')
    groupMatrix = eye(numFeatures); % groupMatrix when non-topo
    isTopo = false;
end
...
for iteration = 1:200                      
    ...
    error = sum(error(:) .^ 2) / batchNumPatches; % error here considers batchNumPatches, actually can be controlled by increasing lambda and gamma 
    
    fResidue = error;
    
    R = groupMatrix * (featureMatrix .^ 2);
    R = sqrt(R + epsilon);    
    fSparsity = lambda * sum(R(:));    % sparsity need not consider batchNumPatches, actually can be controlled by increasing lambda and gamma 
    
    fWeight = gamma * sum(weightMatrix(:) .^ 2);  % weight need not consider batchNumPatches, actually can be controlled by increasing lambda and gamma 
    ...
    % Substep 1: aim to let As ¡Ö x
    featureMatrix = weightMatrix' * batchPatches;             
    normWM = sum(weightMatrix .^ 2)';        % square sum of each column
    % Substep 2: "normalizes" s in an attempt to keep the sparsity penalty small
    featureMatrix = bsxfun(@rdivide, featureMatrix, normWM);  
    
    % substep 3: find the s that minimizes J(A,s) for the A found in the previous step   
    options.maxIter = 20;
    [featureMatrix, cost] = minFunc( @(x) sparseCodingFeatureCost(isTopo, weightMatrix, x, visibleSize, numFeatures, batchPatches, gamma, lambda, epsilon, groupMatrix), ...
                                           featureMatrix(:), options);
    featureMatrix = reshape(featureMatrix, numFeatures, batchNumPatches);                                      
       
    % substep 4: solve for the A that minimizes J(A,s) for the s found in the previous step
    % Optimize for weight matrix, directly derive the result
    weightMatrix = batchPatches*featureMatrix'/(gamma*batchNumPatches* eye(size(featureMatrix, 1)) + featureMatrix*featureMatrix'); 
    
    % therefore this line is useless
