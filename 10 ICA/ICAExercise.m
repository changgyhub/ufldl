...
numPatches = 20000;
numFeatures = 121; % dim of W, i.e. dim of the basis
imageChannels = 3;
patchDim = 8;
visibleSize = patchDim * patchDim * imageChannels; % dim of X, i.e. dim of the input patches
% since we aim to learn an orthogonal basis, numFeatures < visibleSize.                                                   
...
DEBUG = false;
if DEBUG
% Note: numFeatures(dim of W) <= visibleSize(dim of X), numPatches can be chosen arbitrarily
numFeatures = 9;
numPatches = 9;
visibleSize = 12;
patches = patches(1:visibleSize, 1:numPatches);
...
for iteration = 1:10000
    ...
    while 1
        ...
        considerWeightMatrix = real((considerWeightMatrix*considerWeightMatrix')^(-0.5)*considerWeightMatrix);
